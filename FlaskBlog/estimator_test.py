...
import common_test
from common_test import CocoPairsNetwork, CocoPairs, CocoPart
...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            # draw point
            for i in range(common_test.CocoPart.Background.value):
                ...
                centers[i] = center
                cv2.circle(npimg, center, 3, common_test.CocoColors[i], thickness=3, lineType=8, shift=0)

            # draw line
            for pair_order, pair in enumerate(common_test.CocoPairsRender):
                if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                    continue

                npimg = cv2.line(npimg, centers[pair[0]], centers[pair[1]], common_test.CocoColors[pair_order], 3)
